...
\end{proposition}
\begin{remark}
    \Cref{prop:contGen} is a consequence of \Cref{prop:contInv} and the fact that inverse mapping preserves intersection, union and complement.
\end{remark}
...
\section{Outer Measures and Premeasures} %To be checked
...
%    In general, $\bar{\mu}$ is the saturation of the completion of $\mu$.
\end{theorem}
...
\end{proposition}
\begin{remark}
    \Cref{prop:nullDiff} and \ref{prop:negligibleDiff} tells us that modification of $\mu$-measurable functions on negligible sets gives exactly $\bar{\mu}$-measurable functions. However, it is usually enough to modify on null sets, as in the remark following \Cref{prop:limMeasurable}. When considering integration, one can go further to allow a null set or negligible set of points where the function is undefined.
\end{remark}
...
\chapter{Functional Analysis} %To be checked
...
\chapter{Almost Sure Convergence and Laws of Large Numbers}\label{chp:LLN} %To be checked
...
\chapter{Weak Convergence and Central Limit Theorems}\label{chp:CLT} %To be checked
...
\begin{theorem}[Harris' Inequality, \cite{BLM13} Theorem 2.15]
    Given $f,g:\mathbb{R}^n\to \mathbb{R}$, $X=(X_1,\ldots,X_n)$ where $X_i$'s are independent real-valued random variables. If $f$ and $g$ are non-decreasing, then
    \begin{equation}
