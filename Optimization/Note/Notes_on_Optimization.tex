...
\begin{example}
    Though stated in a very general form, mirror descent is basically only used in the $\ell_2$ and $\ell_1$ case. In the $\ell_2$ case, mirror descent corresponds to the common gradient descent while dual averaging corresponds to gradient descent with lazy projection. In the $\ell_1$ case, if in mirror descent we start with uniform distribution, then the two algorithms actually are the same; for convenience the dual averaging version is usually preferred, and is usually called the \textbf{Follow-the-Regularized-Leader} algorithm. Here give a stronger bound for this case, which is extracted from \cite{S11} Theorem 2.22.
\end{example}
...
To analyze the performance, we make use of \Cref{thm:DALocalNorm} and take the randomness into consideration. The key step is that
\begin{equation}
